################################################
#' @description prep STGPR input data for Economics census 
################################################
...
state_data <- data.table(read_feather(paste0(in_dir,"EC_state.feather")))
...
state_data <- state_data[,.(
  ...
  sample_size = .N,
  is_outlier = 0, 
  ds_flag = ifelse(flag_S > 0, flag_S, flag_D) ## flag outlier if we have any data suppression
)]
...
qcew <- data.table(arrow::read_feather("FILEPATH/QCEW_state_covariate.feather"))
