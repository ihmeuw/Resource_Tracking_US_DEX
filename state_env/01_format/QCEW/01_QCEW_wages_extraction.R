...
for (i in files){
  ...
  ## we want aggregated to state/county & owner & industry (varying levels of industry aggregation)
  out <- out[agglvl_code %in% c(54:58, 74:78)]
  
  ## we want to filter to particular industries
  out <- out[industry_code %like% NAICS_parent_regex]
  ...
  ## merge in location demographics (COUNTY)
  setnames(county, "area_fips", "cnty")
  county <- merge(county, counties, by = "cnty")
  setcolorder(county, c("year_id", names(counties)))
  
  ## Currency convert (STATE)
  state[, iso3 := "USA"][, year := year_id]
  state <- currency_conversion(
    data = state, 
    col.loc = "iso3",
    col.value = "total_annual_wages",
    currency = "usd",
  ...
  arrow::write_feather(state, paste0(out_dir, "state/wages/QCEW_", yr, ".feather"))
  arrow::write_feather(county, paste0(out_dir, "county/wages/QCEW_", yr, ".feather"))
}
