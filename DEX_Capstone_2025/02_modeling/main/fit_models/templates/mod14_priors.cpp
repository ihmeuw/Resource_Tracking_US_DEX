// Complex model, with IID on location
// 
...
Type objective_function<Type>::operator() () {
  ...
  nll += SCALE(SEPARABLE(GMRF(K_3j), SEPARABLE(GMRF(Sigma_Sparse_d), GMRF(K_3t))), sigma_3)(re3); 
  ...
  if (link == "poisson") {
    ...
    for(size_t i = 0; i < Y.size(); i++)
      log_m[i] += re1(A[i], D[i], J[i])  + re2[J[i]] + re3(T[i], D[i], J[i]) ;
    ...
    for(size_t i = 0; i < Y.size(); i++)
      log_mu[i] += re1(A[i], D[i], J[i])  + re2[J[i]] + re3(T[i], D[i], J[i]) ;
