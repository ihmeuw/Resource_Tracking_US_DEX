// Complex model, with IID on location (without year)
// 
...
Type objective_function<Type>::operator() () {
  ...
  nll -= dbeta(rho_1j, Type(rho_j_alpha), Type(rho_j_beta), true);
  ...
  nll += SCALE(SEPARABLE(GMRF(K_1j), SEPARABLE(GMRF(Sigma_Sparse_d), GMRF(K_1a))), sigma_1)(re1); 
  
  // RE2: IID(location)
  nll -= dnorm(vector<Type>(re2), Type(0), sigma_2, true).sum();
  
  // RE3: LCAR(time):IID(location)
  // SparseMatrix<Type> K_3t = lcar_strmat(graph_t, rho_3t);  
  // //SparseMatrix<Type> K_3j = lcar_strmat(graph_j, rho_3j);  
  // nll += SCALE(SEPARABLE(GMRF(Sigma_Sparse_j), SEPARABLE(GMRF(Sigma_Sparse_d), GMRF(K_3t))), sigma_3)(re3); 
  
  // Add to nll
  nll -= dnorm(logit_rho_1a, Type(0), Type(1.5), true);
  nll -= dnorm(logit_rho_1j, Type(0), Type(1.5), true);
  //nll -= dnorm(logit_rho_3t, Type(0), Type(1.5), true);
  //nll -= dnorm(logit_rho_3j, Type(0), Type(1.5), true);
  ...
  if (link == "poisson") {
    ...
    for(size_t i = 0; i < Y.size(); i++)
      log_m[i] += re1(A[i], D[i], J[i])  + re2[J[i]] ;
    ...
    for(size_t i = 0; i < Y.size(); i++)
      log_mu[i] += re1(A[i], D[i], J[i])  + re2[J[i]] ;
