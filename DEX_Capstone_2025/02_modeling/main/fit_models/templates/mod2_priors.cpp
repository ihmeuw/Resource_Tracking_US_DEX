...
Type objective_function<Type>::operator() () {
  ...
  nll -= dbeta(rho_2t, Type(rho_t_alpha), Type(rho_t_beta), true);
  ...
  nll += SCALE(SEPARABLE(GMRF(Sigma_Sparse_d), GMRF(K_2t)), sigma_2)(re2);
  
  // Add to nll
  nll -= dnorm(logit_rho_1a, Type(0), Type(1.5), true);
  nll -= dnorm(logit_rho_2t, Type(0), Type(1.5), true);
  ...
  if (link == "poisson") {
    ...
    for(size_t i = 0; i < Y.size(); i++)
      log_m[i] += re1(A[i], D[i]) + re2(T[i], D[i]);
    ...
    for(size_t i = 0; i < Y.size(); i++)
      log_mu[i] += re1(A[i], D[i]) + re2(T[i], D[i]);
