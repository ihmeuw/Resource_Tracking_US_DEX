##----------------------------------------------------------------
## Title: 4_apply_ratios_cause.R
## Purpose: Scale counties requiring additional parallelization on cause, by SHEA ratios
...
t1 <- Sys.time()
dex <- arrow::open_dataset(paste0(compile_dir,'/geo=',geog,'/toc=',care,'/year_id=',yr,'/')) 


if (geog == 'county'){
  message('County level so additionally parallelized on payer: ', pay)
  dex <- dex %>% filter(payer ==pay) %>% filter(acause == cause) 
}
...
if(nrow(dex_scaled) > 0 ){
  ...
  if (run_at_draw_level == T){
    ...
    setkeyv(dex_scaled, by_cols)
    dex_collapsed <- dex_scaled[, .(mean_spend = mean(spend),
                                ...
                                by = by_cols]
    dex_collapsed[,`:=`(year_id = yr, state = st, acause = cause)]
  ...
  arrow::write_dataset(dex_collapsed, path = paste0(final_collapse_dir, "/data/"), 
                       ...
                       basename_template = paste0("year_",yr,"_",cause,"-{i}.parquet"))
  
  
  ##'----------------------------------------------------------------
  ##' 7. IF RUNNING AT DRAW LEVEL, SAVE OUT THE DRAWS
  ##'----------------------------------------------------------------
