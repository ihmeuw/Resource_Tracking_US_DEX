## ==================================================
## Author(s): Max Weil
## Purpose: This script is used to submit Shapley decomposition tasks.
## ==================================================
...
# Defining constants
# Lists of all DEX TOCs and payers
TOCS = ["IP", "AM", "HH", "ED", "NF", "RX"]  # DV
PAYERS = ["mdcr", "mdcd", "priv", "oop"]
YEARS = ["2019"]
...
def run_decomp_submitter(scale_ver, draw_lvl, weighted):
    ...
    # Making dictionaries to run decomp by TOC, payer, and/or cause
    payers = {payer: "payer" for payer in PAYERS}
    ...
    causes = {
        cause: "acause"
        for cause in pd.read_parquet(
            "FILEPATH",
            columns=["acause"],
        )["acause"].unique()
        if cause not in bad_causes
    }
    all_data = {"All": "None"}

    # Setting up Jobmon workflow/tool info
    job_tool = Tool(name="shapley_decomp")
    wf_name = f"shapley_decomp_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}"
    ...
    decomp_task_temp = create_task_temp(
        job_tool,
        task_temp_name="shapley_decomp",
        script_path=Path(__file__).resolve().parent / "helpers" / "shapley_decomp.R",
        script_args=[
            "scale_ver",
            "draw",
            "year",
    ...
    decomp_task_specs = list(
        itertools.product(
            draws, YEARS, list(dict(**all_data, **payers, **tocs, **causes).items())
        )
    )

    # Creating data prep task for each year and state
    decomp_task_list = []
    for draw, year, (filter_val, filter_col) in decomp_task_specs:
    ...
    print(f"Running Shapley decomposition workflow.")
